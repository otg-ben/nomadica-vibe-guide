import type { VibeGuide } from '@/data/mendocino'
...
interface Props {
  ...
  heroAlt: string
  photos?: GalleryPhoto[]
  roadNetworkImage?: string
  roadNetworkImageAlt?: string
}

export default function VibeGuidePage({
  data: g,
  heroImage,
  heroAlt,
  photos,
  roadNetworkImage,
  roadNetworkImageAlt = 'Road network',
}: Props) {
  const placeMap = Object.fromEntries(g.places.map(p => [p.id, p.name]))
          ...
          <div className="space-y-5">
            {roadNetworkImage && (
              <div className="rounded-xl overflow-hidden h-36">
                <img src={roadNetworkImage} alt={roadNetworkImageAlt} className="w-full h-full object-cover" />
              </div>
            )}
        ...
        <Section title="Recreational Opportunities">
          <div className="space-y-3">
            {g.recOpportunities.map(rec => (
              <RecCard key={rec.name} rec={rec} placeMap={placeMap} />
            ))}
          </div>
        </Section>
